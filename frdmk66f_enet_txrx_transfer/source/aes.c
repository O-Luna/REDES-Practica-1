...
size_t encrypt_message(const uint8_t *message, uint32_t length, uint8_t *cypher)
{
	...
	if (length > 112) return 0;  // Seguridad: evitar desbordamiento
	...
	uint32_t padded_len = length + pad;
    ...
    // Copiar a salida
    memcpy(cypher, buffer, padded_len);
...
size_t decrypt_message(const uint8_t *cypher, uint32_t length, uint8_t *message)
{
	...
	// Verificar y eliminar padding PKCS#7
	uint8_t pad = buffer[length - 1];
	if (pad == 0 || pad > AES_BLOCKLEN) return 0;

	uint32_t unpadd_len = length - pad;
	memcpy(message, buffer, unpadd_len);
